...
- **`@graphql2mcp/core`** -- schema loading (SDL, files, globs, introspection JSON, live URL introspection), GraphQL-to-Zod type mapping, tool name generation, selection set building, the `generateTools` orchestrator, runtime GraphQL execution (`executeGraphQL`), and the high-level `getGraphQLTools` / `registerGraphQLTools` API.
