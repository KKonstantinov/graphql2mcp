---
title: CLI Reference
description: The graphql2mcp CLI converts a GraphQL endpoint or schema file into a standalone MCP server.
---
...
See the [Mutations guide](/docs/mutations) for details on mutation modes and tool annotations.
...
The server starts an HTTP server using the MCP Streamable HTTP transport. This is the recommended transport for most use cases — remote
clients, web-based agents, debugging, and production deployments.
...
The server communicates over stdin/stdout using the MCP stdio transport. Use this for desktop MCP clients like Claude Desktop and Cursor
...
When the source is a file, glob, or SDL string, you must specify the execution endpoint with `--endpoint` (`-e`). This is the URL where
GraphQL queries will be sent at runtime.
